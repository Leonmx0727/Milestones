...
// Admin list of leagues with zero user associations
...
switch ($sort){
  ...
  default:            $orderBy='ORDER BY l.created DESC, l.id DESC';
}

try {
  $cs = "SELECT COUNT(*) FROM leagues l LEFT JOIN user_league_follows ulf ON ulf.league_id=l.id $whereSql";
  $stmt = db()->prepare($cs); $stmt->execute($params); $total=(int)$stmt->fetchColumn();
} catch (Throwable $e) { error_log('leagues_unassociated count: '.$e->getMessage()); $total=0; }

try {
  $sql = "SELECT l.* FROM leagues l
          LEFT JOIN user_league_follows ulf ON ulf.league_id=l.id
          $whereSql $orderBy LIMIT $limit OFFSET $offset";
  $stmt = db()->prepare($sql); $stmt->execute($params); $rows=$stmt->fetchAll();
} catch (Throwable $e) { error_log('leagues_unassociated fetch: '.$e->getMessage()); $rows=[]; }
$shown = count($rows);
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Admin • Unassociated Leagues</title>
...
<div class="container">
  ...
  <form method="get" onsubmit="return validateAdminAssocFilters(this)" style="margin-bottom:12px;display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(220px,1fr))">
    ...
    <div><button type="submit">Apply Filters</button><a class="button secondary" href="<?= BASE_URL ?>/pages/leagues_unassociated.php" style="margin-left:8px;display:inline-block;padding:12px 16px;background:#6c757d;color:#fff;border-radius:8px;text-decoration:none;">Reset</a></div>
  </form>
  ...
  <?php else: ?>
    <div style="overflow:auto">
      <table style="width:100%;border-collapse:collapse">
        <thead><tr><th style="padding:8px">Logo</th><th style="padding:8px">League</th><th style="padding:8px">Country</th><th style="padding:8px">Type</th><th style="padding:8px">Current</th><th style="padding:8px">Actions</th></tr></thead>
        <tbody>
        <?php foreach ($rows as $r): ?>
          <tr style="border-bottom:1px solid #f0f0f0">
            ...
            <td style="padding:8px;white-space:nowrap"><a href="<?= BASE_URL ?>/pages/leagues_view.php?id=<?= (int)$r['id'] ?>">View</a></td>
          </tr>
        <?php endforeach; ?>
        </tbody>
      </table>
    ...
    <?php
      $base = BASE_URL . '/pages/leagues_unassociated.php?' . http_build_query(array_merge($_GET, ['page'=>null]));
